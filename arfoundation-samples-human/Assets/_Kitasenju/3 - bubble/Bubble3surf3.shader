...
Shader "custom/Bubble3surf3" {
	...
	SubShader {
        ...
        void vert(inout appdata_full v, out Input o )
        {
			...
			float3 gp = mul(unity_ObjectToWorld, float3(0,0,0)) + _Time.y;// + _Random*100;
			...
			//ワールド座標
                ...
                half3 reflectDir		= reflect(viewDir, worldNormal);//カメラから表面に反射してキューブマップを得る用
		...
		void surf (Input IN, inout SurfaceOutputStandard o ) {
			...
			float ref =  dot(IN.viewDir, o.Normal);//normalはワールドノーマル（多分）

            fixed4 col = texCUBE (_Cube, IN.reflectDir);//tex2D (_MainTex, IN.uv_MainTex);

			if( ref<0 ){
				//裏
				col = texCUBE (_Cube, -IN.reflectDir);
			}

			float2 uvv = frac( IN.uv_MainTex + float2(_Time.x,_Time.x*1.1+_Random) );
			fixed4 rainbow = tex2D (_MainTex, uvv);

			// Albedo comes from a texture tinted by color
			fixed4 c = max( col,rainbow );//,tex2D (_GrabPassTexture, IN.samplingViewportPos) );
			o.Albedo = c.rgb * 0.05;
			o.Emission = c.rgb * 0.95;
			// Metallic and smoothness come from slider variables
			o.Metallic = _Metallic;
			o.Smoothness = _Glossiness;

			if( ref>0 ){
				o.Alpha = saturate( 1 - ref*_Pow );
				//o.Alpha = smoothstep(0,0.1,o.Alpha);
				//o.Alpha = pow(o.Alpha,_Pow);
			}else{
				//裏
				o.Alpha = 0.5 * ( 1 + ref );
			}
