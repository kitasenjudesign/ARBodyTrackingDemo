...
#define _ColorA_arr Props
        ...
        float4 getNewVertPosition( float4 v, float delay )
		{
            float3 vv = v.xyz;
			float amp = length(vv);
			...
			float dAmp	= snoise( vv.xyz*_Detail + _Time.y + delay ) * _Amount;            
			float dRadX	= sin( vv.y*_Detail2 + _Time.z + delay ) * _RotAmount;//横方向の角度
        ...
        void vert(inout appdata_full v, out Input o )
        {
			...
			float4 vertPosition = getNewVertPosition( v.vertex, delay );
			...
			float4 v1 = getNewVertPosition( v.vertex + v.tangent * vertOffset, delay );
			float4 v2 = getNewVertPosition( v.vertex + bitangent * vertOffset, delay );
            ...
            float4 vv = UnityObjectToClipPos(v.vertex);
            o.pos = vv;//floor(vv.xy*10.0) / 10.0;
            o.pos.xy = floor( vv.xy * 10 ) / 10;
