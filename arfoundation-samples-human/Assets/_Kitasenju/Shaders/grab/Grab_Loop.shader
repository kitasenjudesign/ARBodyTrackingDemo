    ...
    SubShader {
        ...
        Pass
        {
            ...
            half4 frag( v2f i ) : COLOR
            {
                ...
                half4 col = tex2D( _BackgroundTexture, screenPos );
                
                float loop = rand( floor( screenPos * float2(1,aspct) * 30 ) / 30 ) * 20;

                loop = clamp( loop, 0.0, 20.0 );

                col.rgb = frac( col.rgb * loop );
