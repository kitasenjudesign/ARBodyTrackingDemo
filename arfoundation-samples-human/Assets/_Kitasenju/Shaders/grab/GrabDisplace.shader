    ...
    SubShader {
        ...
        Pass
        {
            ...
            half4 frag( v2f i ) : COLOR
            {
                ...
                //screenPos = floor( aspct * screenPos * _Mosaic ) / _Mosaic;
                ...
                float2 offsetR = float2(
                    snoise( float3(i.uv*2+screenPos.xy*_Detail.x,_Time.x*_Speed.x) ) * ss.x,
                    snoise( float3(i.uv*2+screenPos.yx*_Detail.y,_Time.x*_Speed.x) ) * ss.y
                );
                float2 offsetG = float2(
                    snoise( float3(i.uv*2+screenPos.xy*_Detail.x,_Time.x*_Speed.y) ) * ss.x,
                    snoise( float3(i.uv*2+screenPos.yx*_Detail.y,_Time.x*_Speed.y) ) * ss.y
                );
                float2 offsetB = float2(
                    snoise( float3(i.uv*2+screenPos.xy*_Detail.x,_Time.x*_Speed.z) ) * ss.x,
                    snoise( float3(i.uv*2+screenPos.yx*_Detail.y,_Time.x*_Speed.z) ) * ss.y
                );
                float2 offsetA = float2(
                    snoise( float3(i.uv*2+screenPos.xy*_Detail.x,_Time.x*_Speed.w) ) * ss.x,
                    snoise( float3(i.uv*2+screenPos.yx*_Detail.y,_Time.x*_Speed.w) ) * ss.y
                );
